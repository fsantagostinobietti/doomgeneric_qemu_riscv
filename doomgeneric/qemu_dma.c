...
int qemu_cfg_find_file() {
    /* see https://github.com/qemu/qemu/blob/master/docs/specs/fw_cfg.rst#file-directory-key-0x0019-fw_cfg_file_dir
    */
    ...
    //kprintf("qemu_cfg_find_file(): select = [%d]\n", select);
    return select;
}
...
int check_fw_cfg_dma() {
    ...
    and reading four bytes from the data register. 
    If the fw_cfg device is present, the four bytes read will contain the characters QEMU.
    ...
    (see https://github.com/qemu/qemu/blob/master/docs/specs/fw_cfg.rst#signature-key-0x0000-fw_cfg_signature)
    */
